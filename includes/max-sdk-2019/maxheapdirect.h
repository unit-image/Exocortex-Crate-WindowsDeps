...
//! \deprecated Obsolete function as of 3ds Max 2019 due to changes in the underlying CRT.  Check directly for null pointer instead.
MAXMEM_EXTERN_C MAX_DEPRECATED UtilExport int	(__cdecl *MAX_CrtIsValidPointer)(const void *address,unsigned int size,int access);

//Verify that a specified pointer is in the local heap
//! \deprecated Obsolete function as of 3ds Max 2019 due to changes in the underlying CRT.  Check directly for null pointer instead.
MAXMEM_EXTERN_C MAX_DEPRECATED UtilExport int	(__cdecl *MAX_CrtIsValidHeapPointer)(const void *userData);
